...
# Define existing tools (copy from your original mcp_jira_server21.py)
@mcp.tool()
    ...
    try:
        ...
        for field in fields:
            if field.get("name") == "Epic Name":
                return field["id"]
        return "customfield_10011"  # Fallback
    ...
    try:
        ...
        if filenames:
            ...
            for filename in filenames:
                matching_attachment = next((att for att in attachments if att["filename"] == filename), None)
            ...
            for attachment in attachments:
                ...
                saved_files.append(file_path)
                logger.debug(f"Downloaded attachment: {file_path}")
        message = (f"Downloaded {len(saved_files)} of {len(filenames)} requested files from issue {issue_key}. Not found: {not_found_files}" if filenames
                   else f"Successfully downloaded {len(saved_files)} attachments from issue {issue_key}")
        return {
...
async def handle_mcp_request(request: dict):
    ...
    try:
        # Assuming FastMCP has a method to process JSON-RPC requests
        # If process_request doesn't exist, implement custom logic
        response = mcp._handle_jsonrpc_request(request)  # Adjust based on actual FastMCP API
        return response
    except Exception as e:
